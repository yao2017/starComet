...
namespace netService.AuthHelper
{
    /// <summary>
    /// Token验证授权中间件
    /// </summary>
    public class TokenAuth
    {
        ...
        public Task Invoke(HttpContext httpContext)
        {
            ...
            try
            {
                ...
                //验证缓存中是否存在该jwt字符串
                if (!MemoryCacheModel.Exists(jwtStr))
                {
                    return httpContext.Response.WriteAsync("非法访问");
                }
                TokenModel tm = ((TokenModel)MemoryCacheModel.Get(jwtStr));
